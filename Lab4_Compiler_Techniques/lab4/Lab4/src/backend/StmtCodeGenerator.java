...
public class StmtCodeGenerator extends Visitor<Void> {
	...
	public Void visitBreakStmt(BreakStmt nd) {
	    ...
	    Unit breakStmt = breakTargets.get(whileStmt); // this. breakTargetUntit needed?
	    units.add(j.newGotoStmt(breakStmt)); // this. breakTargetUntit needed?
	    return null;
	  }
	...
	public Void visitWhileStmt(WhileStmt nd) {
		...
		// If cond == 0(false) go label 1
		units.add(j.newIfStmt(j.newEqExpr(condition, IntConstant.v(0)), label1));
