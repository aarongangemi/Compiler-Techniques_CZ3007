...
public class ExprCodeGenerator extends Visitor<Value> {
	...
	public Value visitArrayIndex(ArrayIndex nd) {
		...
		// this.wrap or wrap?
		return Jimple.v().newArrayRef(wrap(nd.getBase().accept(this)), wrap(nd.getIndex().accept(this)));
	}
	...
	public Value visitBinaryExpr(BinaryExpr nd) {
		...
		Value result = nd.accept(new Visitor<Value>() {
			@Override
			public Value visitAddExpr(AddExpr value)
			{
				return Jimple.v().newAddExpr(left,right);
			}
			@Override
			public Value visitSubExpr(SubExpr value)
			{
				return Jimple.v().newSubExpr(left,right);
			}
			@Override
			public Value visitMulExpr(MulExpr value)
			{
				return Jimple.v().newMulExpr(left,right);
			}
			@Override
			public Value visitDivExpr(DivExpr value)
			{
				return Jimple.v().newDivExpr(left,right);
			}
			@Override
			public Value visitModExpr(ModExpr value)
			{
